<?php
    ...
    if(empty(trim($name)) || empty(trim($address)) || empty(trim($email)) || empty(trim($tel))){
    ...
    else if(!preg_match("/^[กขฃคฅฆงจฉชซฌญฎฏฐฑฒณดตถทธนบปผฝพฟภมยรลวศษสหฬอฮฯะาิีึืุูเะเแะแโะโเาะเะำใไฤ๋้ัa-zA-Z ]+$/",$name)){ //ต้องมีเว้นวรรคด้วย
        ...
        mysqli_query($conn, $sql) or die("ไม่สามารถเลือกฐานข้อมูล bookstore ได้".mysqli_error());
        header('location:insertform.php');
        // echo '<br><br><h2>บันทึกข้อมูลลุกค้าชื่อ '.$name.' สำเร็จ</h2>';
